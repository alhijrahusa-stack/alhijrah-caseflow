      ...
      async function api(path, opt = {}) {
        ...
        if (!r.ok) {
          ...
          if (j.details) error.details = j.details;
          throw error;
        }
        return j;
      }
      ...
      async function editCase(id){
        try{
          const[result,evidenceResult]=await Promise.all([api(`/api/v1/cases/${id}/workspace`),api(`/api/v1/cases/${id}/agency-requests`)]),workspace=result.data,c=workspace.case,client=workspace.client||{};workspace.evidence=evidenceResult.data;
      ...
      function openForm(){$("formErr").textContent="";$("formModal").classList.add("show")}
      ...
      async function createAgencyRequest(){const request_type=prompt(currentLanguage==="Arabic"?"نوع الطلب: RFE أو RFIE أو NOID أو NOIR":"Request type: RFE, RFIE, NOID, or NOIR","RFE");if(!request_type)return;const title=prompt(currentLanguage==="Arabic"?"عنوان إشعار الجهة":"Agency notice title");if(!title)return;const response_due_date=prompt(currentLanguage==="Arabic"?"تاريخ استحقاق الرد YYYY-MM-DD (اختياري)":"Response due date YYYY-MM-DD (optional)","");try{await api(`/api/v1/cases/${currentWorkspace.case.id}/agency-requests`,{method:"POST",body:JSON.stringify({request_type,title,response_due_date:response_due_date||null})});await editCase(currentWorkspace.case.id);switchWorkspaceTab("rfe")}catch(error){alert(error.message)}}
      async function createEvidenceRequirement(requestId){const requirement_code=prompt(currentLanguage==="Arabic"?"رمز بند الدليل":"Evidence item code");if(!requirement_code)return;const title=prompt(currentLanguage==="Arabic"?"الدليل المطلوب":"Required evidence");if(!title)return;try{await api(`/api/v1/agency-requests/${requestId}/requirements`,{method:"POST",body:JSON.stringify({requirement_code,title})});await editCase(currentWorkspace.case.id);switchWorkspaceTab("rfe")}catch(error){alert(error.message)}}
      async function linkEvidenceDocument(requirementId){const document_id=$(`evidence-doc-${requirementId}`)?.value;if(!document_id)return alert(currentLanguage==="Arabic"?"اختر مستندًا.":"Select a document.");try{await api(`/api/v1/evidence-requirements/${requirementId}/documents`,{method:"POST",body:JSON.stringify({document_id})});await editCase(currentWorkspace.case.id);switchWorkspaceTab("rfe")}catch(error){alert(error.message)}}
      async function setEvidenceStatus(requirementId){const status=prompt(currentLanguage==="Arabic"?"الحالة: missing, requested, received, accepted, insufficient, waived":"Status: missing, requested, received, accepted, insufficient, waived","accepted");if(!status)return;try{await api(`/api/v1/evidence-requirements/${requirementId}`,{method:"PATCH",body:JSON.stringify({status:status.toLowerCase()})});await editCase(currentWorkspace.case.id);switchWorkspaceTab("rfe")}catch(error){alert(error.message)}}
        ...
        try {
          ...
          $("docTable").innerHTML =
            docs
              .map(
                (d) =>
                  `<tr><td><b>${esc(d.file_name)}</b><small class="u-66a84ce714">${esc(intakeOptionLabel(d.category||"unclassified"))} · v${Number(d.version||1)}</small></td><td>${esc(names[d.case_id] || "Unknown")}</td><td>${esc(d.content_type || "—")}</td><td>${formatSize(d.size_bytes)}</td><td>${date(d.created_at)}</td><td>${documentActions(d)}</td></tr>`,
              )
              .join("") ||
            '<tr><td colspan="6">No documents uploaded.</td></tr>';
          $("documentGrid").innerHTML=docs.map(d=>`<article class="document-card"><span class="tag">${esc(intakeOptionLabel(d.automation_status||d.review_status||"received"))}</span><b>${esc(d.file_name)}</b><small>${esc(names[d.case_id]||"Unknown")} · ${esc(intakeOptionLabel(d.category||"unclassified"))}</small><small>${formatSize(d.size_bytes)} · v${Number(d.version||1)} · ${date(d.created_at)}</small><div>${documentActions(d)}</div></article>`).join("")||`<div class="empty">${esc(tr("noDocuments"))}</div>`;
          $("documentGrid").hidden=!documentGridMode;$("docTable").closest(".table").hidden=documentGridMode;
        } catch (e) {
          $("docErr").textContent = e.message;
        }
      }
      function documentActions(document){const actions=[];if(["application/pdf","image/jpeg","image/png","image/webp"].includes(document.content_type))actions.push(`<button class="linkbtn" data-act="previewDoc" data-a1="${esc(document.id)}">Preview</button>`);actions.push(`<button class="linkbtn" data-act="downloadDoc" data-a1="${esc(document.id)}">Download</button>`);if(["image/jpeg","image/png","image/webp"].includes(document.content_type)&&(allowedTo("documents.manage")||allowedTo("documents.review")))actions.push(`<button class="linkbtn" data-act="reviewDocumentOcr" data-a1="${esc(document.id)}">OCR</button>`);if(allowedTo("documents.manage")&&document.automation_status==="FAILED")actions.push(`<button class="linkbtn" data-act="retryDocumentProcessing" data-a1="${esc(document.id)}">Retry processing</button>`);if(allowedTo("documents.manage")){actions.push(`<button class="linkbtn" data-act="replaceDoc" data-a1="${esc(document.id)}">Replace</button>`);actions.push(`<button class="linkbtn" data-act="deleteDoc" data-a1="${esc(document.id)}">Archive</button>`)}return actions.join(" · ")}
      ...
      async function retryDocumentProcessing(id){try{await api(`/api/v1/documents/${id}/retry-processing`,{method:"POST",body:"{}"});await loadDocuments();if(currentWorkspace?.case?.id)await editCase(currentWorkspace.case.id)}catch(error){alert(error.message)}}
