...
const productionIdentityMrz=Object.freeze(['P<USAERIKSSON<<ANNA<MARIA<<<<<<<<<<<<<<<<<<<','L898902C36USA7408122F1204159<<<<<<<<<<<<<<<8']);
async function productionIdentityImage(){const sharp=(await import('sharp')).default;const escapeMrz=value=>value.replaceAll('<',String.fromCharCode(38)+'lt;');const svg='<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="800"><rect width="1800" height="800" fill="white"/><text x="60" y="120" font-family="DejaVu Sans Mono" font-size="56" fill="black">PASSPORT</text><text x="60" y="220" font-family="DejaVu Sans Mono" font-size="38" fill="black">UNITED STATES OF AMERICA</text><text x="60" y="650" font-family="DejaVu Sans Mono" font-size="47" letter-spacing="1" fill="black">'+escapeMrz(productionIdentityMrz[0])+'</text><text x="60" y="730" font-family="DejaVu Sans Mono" font-size="47" letter-spacing="1" fill="black">'+escapeMrz(productionIdentityMrz[1])+'</text></svg>';return sharp(Buffer.from(svg)).png().toBuffer()}
...
async function handle(req,res){
  ...
  if(req.method==='GET'&&u.pathname==='/ready'){
    const [authStatus,databaseState,r2State]=await Promise.all([
      getAuthProvisioningStatus(),
      (async()=>{
        const state={connected:false,coreSchema:false,phase1Schema:false,authorizationSchema:false,authorizationTables:{teams:false,teamMembers:false,accessPolicies:false,recordAccessGrants:false},authorizationTableErrors:{}};
    ...
    const checks={supabase:databaseState.connected,coreSchema:databaseState.coreSchema,phase1Schema:databaseState.phase1Schema,authorizationSchema:databaseState.authorizationSchema,r2:r2State,internalAuth:Boolean(internalApiKey),userAuth:authStatus.configured,ownerAccount:authStatus.ownerProvisioned,emailDelivery:Boolean(process.env.RESEND_API_KEY&&process.env.RESEND_FROM_EMAIL)};
  ...
  if(req.method==='GET'&&u.pathname==='/api/v1/settings/office'){
    const settings=await officeSettings();
    return json(res,200,{data:{...settings,logo_url:settings.logo_object_key?'/brand/logo':null,email_provider_configured:Boolean(process.env.RESEND_API_KEY&&process.env.RESEND_FROM_EMAIL)},requestId},ch);
  }
