...
async function validateAnswerProvenance(caseRecord,body){
  ...
  if(sourceType==='manual'||sourceType==='system')return;
  ...
  else if(sourceType==='participant'||sourceType==='verified_field')rows=await db('case_people',{query:`?case_id=eq.${caseRecord.id}&person_id=eq.${sourceId}&select=person_id&limit=1`});
...
async function processImport(batchId,principal){
  if(runningImports.has(batchId))return;runningImports.add(batchId);
  ...
  }catch(error){await db('import_batches',{method:'PATCH',query:`?id=eq.${batchId}`,body:{status:'failed',error_code:String(error.message||'IMPORT_FAILED').slice(0,120),updated_at:new Date().toISOString()}}).catch(()=>{});console.error('background-import-failed',batchId,error.message);}finally{runningImports.delete(batchId);}}
function scheduleImport(batchId,principal){setImmediate(()=>processImport(batchId,{id:principal.id,displayName:principal.displayName,roles:principal.roles}));}
...
async function handle(req,res){
  ...
  if(u.pathname==='/api/v1/profile/preferences'&&(req.method==='GET'||req.method==='PATCH')){
    ...
    const profilePrincipal=await resolveApplicationPrincipal(await authenticateSession(req,res));
  ...
  if(req.method==='GET'&&u.pathname==='/api/v1/search'){
    ...
    if(term.length<2)return json(res,200,{data:{clients:[],cases:[]},requestId},ch);
    ...
    return json(res,200,{data:{clients:visibleClients,cases:filterAccessibleCases(access,caseRows||[],'cases.view')},requestId},ch);
  }
...
export function createServer(){
  ...
  db('import_batches',{query:'?status=in.(approved,processing)&select=id,uploaded_by&limit=100'}).then(rows=>rows.forEach(row=>scheduleImport(row.id,{id:row.uploaded_by,displayName:'Import recovery',roles:['system']}))).catch(error=>{if(!isMissingRelation(error))console.error('import-recovery-failed',error.message)});
