...
async function handleRaw(req,res){
  ...
  if(taskMatch&&req.method==='PATCH'){
    ...
    const body=await readJson(req);
    if(body.case_id!==undefined&&String(body.case_id||'')!==String(rows[0].case_id||''))throw Object.assign(new Error('TASK_REASSIGNMENT_NOT_ALLOWED'),{status:400});
    if(body.client_id!==undefined&&String(body.client_id||'')!==String(rows[0].client_id||''))throw Object.assign(new Error('TASK_REASSIGNMENT_NOT_ALLOWED'),{status:400});
    const record=normalizeTaskInput({...rows[0],...body,case_id:rows[0].case_id,client_id:rows[0].client_id});
