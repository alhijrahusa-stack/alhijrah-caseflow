...
test('authorized staff can idempotently restart an exhausted document job',async()=>{
  const cookie=await signIn(user.email),headers=browserHeaders({cookie}),caseId=crypto.randomUUID(),documentId=crypto.randomUUID(),jobId=crypto.randomUUID(),checksum='d'.repeat(64),uploader=crypto.randomUUID();
  backend.tables.cases.push({id:caseId,client_id:clientId,client_name:'Convergence Client',case_type:'N-400',status:'active',archived_at:null});backend.tables.documents.push({id:documentId,case_id:caseId,client_id:clientId,object_key:`cases/${caseId}/failed.pdf`,file_name:'failed.pdf',content_type:'application/pdf',size_bytes:10,content_checksum:checksum,version:1,uploaded_by:uploader,automation_status:'FAILED',processing_error:'DOCUMENT_EXTRACTION_FAILED',archived_at:null});backend.tables.background_jobs.push({id:jobId,job_type:'DOCUMENT_EXTRACT',idempotency_key:`document-extract:${documentId}:v1:${checksum}`,case_id:caseId,status:'failed',progress:0,payload:{document_id:documentId,document_version:1,source_sha256:checksum},attempt_count:5,max_attempts:5,priority:100,requested_by:uploader,input_fingerprint:'0'.repeat(64),created_at:new Date().toISOString()});
  const retried=await request({method:'POST',path:`/api/v1/documents/${documentId}/retry-processing`,headers,body:{}});assert.equal(retried.status,202,retried.raw);assert.equal(retried.body.data.job_id,jobId);assert.equal(backend.tables.background_jobs[0].status,'retrying');assert.equal(backend.tables.background_jobs[0].attempt_count,0);assert.equal(backend.tables.documents[0].automation_status,'QUEUED');assert.equal(backend.tables.documents[0].processing_error,null);assert.ok(backend.tables.audit_events.some(item=>item.action==='document_processing_retried'&&item.entity_id===caseId&&item.metadata.document_id===documentId));
  backend.tables.documents[0].automation_status='FAILED';const repeated=await request({method:'POST',path:`/api/v1/documents/${documentId}/retry-processing`,headers,body:{}});assert.equal(repeated.status,200,repeated.raw);assert.equal(repeated.body.data.idempotent,true);assert.equal(backend.tables.background_jobs.length,1);
});

test('translator upload authority is limited to an explicit translation request',async()=>{
  const translator=addUser({email:'translator@caseflow.test',roles:['translator']}),cookie=await signIn(translator),headers=browserHeaders({cookie,'content-type':'application/pdf'}),caseId=crypto.randomUUID(),requestId=crypto.randomUUID();
  backend.tables.cases.push({id:caseId,client_id:clientId,client_name:'Convergence Client',case_type:'Document Translation',service_code:'TRANSLATION',status:'active',archived_at:null});backend.tables.document_requests.push({id:requestId,case_id:caseId,client_id:clientId,requirement_code:'SOURCE_DOCUMENTS',category:'translation',status:'missing'});
  const bytes=Buffer.from('%PDF-1.4\nverified translation output\n%%EOF'),uploaded=await request({method:'POST',path:`/api/v1/documents/upload?case_id=${caseId}&request_id=${requestId}&category=translation&filename=translation.pdf&size_bytes=${bytes.length}`,headers,body:bytes});
  assert.equal(uploaded.status,201,uploaded.raw);assert.equal(uploaded.body.data[0].category,'translation');assert.equal(uploaded.body.data[0].request_id,requestId);assert.equal(backend.tables.document_requests[0].status,'received');
  const unrelated=Buffer.from('%PDF-1.4\nunrequested output\n%%EOF'),denied=await request({method:'POST',path:`/api/v1/documents/upload?case_id=${caseId}&category=translation&filename=unrequested.pdf&size_bytes=${unrelated.length}`,headers,body:unrelated});assert.equal(denied.status,409,denied.raw);assert.equal(denied.body.error,'TRANSLATION_REQUEST_REQUIRED');assert.equal(backend.tables.documents.length,1);
});
...
test('an authorized reviewer commits background extraction and refreshes affected form answers',async()=>{
  ...
  const extractionQueue=await request({path:`/api/v1/documents/${documentId}/ocr`,headers});
  assert.equal(extractionQueue.status,200,extractionQueue.raw);assert.equal(extractionQueue.body.data[0].id,extractionId);
  const managerCookie=await signIn(manager),managerDenied=await request({method:'POST',path:`/api/v1/documents/${documentId}/ocr/confirm`,headers:browserHeaders({cookie:managerCookie}),body:{confirmed:true,review_token:'not-a-review-token',fields:{legal_name:'Unreviewed'}}});
  assert.equal(managerDenied.status,403,managerDenied.raw);assert.equal(backend.tables.document_extractions[0].status,'pending_review');
