...
async function handleRest(url, init) {
  ...
  if(table==='rpc/commit_verified_identity_extraction'){
    ...
    if(extraction.document_id){
      const document=backend.tables.documents.find(item=>item.id===extraction.document_id&&item.case_id===extraction.case_id&&Number(item.version||1)===Number(extraction.document_version||1)&&item.content_checksum===extraction.source_sha256&&!item.archived_at);
      if(!document)return jsonResponse(409,{message:'Confirmed extraction source document is unavailable'});
      Object.assign(document,{automation_status:'VERIFIED',review_status:'approved',reviewer_notes:null,reviewed_by:actor,reviewed_at:reviewedAt,processing_error:null});
      if(document.request_id){const request=backend.tables.document_requests.find(item=>item.id===document.request_id&&item.case_id===document.case_id);if(request)Object.assign(request,{status:'approved',reviewed_by:actor,reviewer_notes:null,updated_at:reviewedAt})}
      const task=backend.tables.tasks.find(item=>item.automation_key===`document:${document.id}:v${Number(document.version||1)}`);if(task)Object.assign(task,{status:'completed',completed_at:reviewedAt,updated_by:actor,updated_at:reviewedAt});
    }
